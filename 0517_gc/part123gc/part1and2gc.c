...
enum { LIST=0, SYMBOL=1, FREE=2, MARK_BIT=0x100 };
int dbg=0;
...
/* 読み込み用下請 */
Ptr new_ptr();
//  Ptr p=(Ptr)malloc(sizeof(struct ptr));
//  if( p==NULL ) exit(1);   /* メモリが尽きたら終了 */
//  return p;
//}

Ptr new_symbol(const char*pname){extern char* strdup(const char*); /* アトムのための領域確保 */
  Ptr x=new_ptr();
  x->tag=SYMBOL;
...
/* 全てのアトムはシステム内でユニーク！ */
Ptr intern(const char*s){ /* 全てのアトムをリストにして登録 */
  Ptr p,id;
...
int nextchar; int flag=0;
int mygetchar(){int ch;
  if(flag){ flag=0; return nextchar; }
  else { ch=getchar(); return ch; } }
int peekchar(){
  if(flag){ return nextchar; }
  else { nextchar=mygetchar(); flag=1; return nextchar; } }
...
Ptr read_expr_list(int n){
  ...
  if( tk==')' ) { return nil_pointer; }
  if( tk=='.' ) {
    if( n==0 ) {fprintf(stderr,"Format error(1)\n"); exit(1);}
    Ptr p=read_expr();
    if( next_token()!=')' ){fprintf(stderr,"Format error(2)\n");exit(1);}
    return p;
  }
...
Ptr p_apply(Ptr fn, Ptr args, Ptr a){
  ...
  if( car(fn)==lambda_pointer )
    return p_eval( car(cdr(cdr(fn))),
		   p_pairlis(car(cdr(fn)), args, a));
  if( car(fn)==label_pointer )
    return p_apply(car(cdr(cdr(fn))), args,
		   cons( cons(car(cdr(fn)),car(cdr(cdr(fn)))), a));
  printf("unknown functions: "); print_expr(fn); printf("\n");
  return nil_pointer;
}
/* eval */
Ptr p_eval(Ptr e, Ptr a){
  ...
  if( symbolp(e)==true_pointer ){
    ...
    printf("unbound variable: "); print_expr(e); printf("\n");
    return nil_pointer;
  }
...
Ptr p_evcon(Ptr c, Ptr a){
  ...
  if( p_eval(car(car(c)),a)!=nil_pointer )
    return p_eval(car(cdr(car(c))), a);
...
int main(){
  ...
  lisp_env=cons( cons( true_pointer, true_pointer ),
                 cons( cons(nil_pointer,nil_pointer),
                       nil_pointer) );
  while(1){ Ptr x,y;
    printf("\nlisp> ");
    x=read_expr();
