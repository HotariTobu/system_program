...
#ifndef UNDEF_GSP
long long int GET_STACK_PTR(void*x){ long long int p=(long long int)&x;
    return p;
}
#else
long long int GET_STACK_PTR(void*x);
#endif
...
{   int i;
    heap_area = (Ptr)calloc(max_cell_no,sizeof(struct ptr));
...
int gc_init() {           /* GCのための初期化. 起動時のスタックを保存 */
    sp_init = (void**)GET_STACK_PTR(env_init);
    return 0;
}

int gc(){               /* GCのプログラム入り口 */
  extern int gc_sys();
  sp_cur = (void**)GET_STACK_PTR(env_cur);
  return gc_sys();
}
...
int gc_sys(){              /* GCの本体  */
    ...
    /* mark from stack : スタックから辿れるものをマーク */
    {
      ...
      tmp=sp_cur;
      while(n-->=0){
        if( in_heap_area(*tmp) ) dfs_mark((Ptr)*tmp);
        tmp++;
      }
    }
